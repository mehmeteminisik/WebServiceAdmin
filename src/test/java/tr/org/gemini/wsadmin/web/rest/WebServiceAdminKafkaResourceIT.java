...
import tr.org.gemini.wsadmin.config.KafkaProperties;

class WebServiceAdminKafkaResourceIT {
    ...
    void setup() {
        ...
        WebServiceAdminKafkaResource kafkaResource = new WebServiceAdminKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/web-service-admin-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/web-service-admin-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
